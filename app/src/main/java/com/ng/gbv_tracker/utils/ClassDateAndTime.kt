...
// it comes out like this 2013-08-31 15:55:22 so adjust the date format
    ...
    fun getMonth(timeStamp: Long): String? {
        return try {
            val sdf = SimpleDateFormat("MMM dd", Locale.getDefault())//Locale.US for usa
            val netDate = Date(timeStamp*1000)
            sdf.format(netDate)
        } catch (e: Exception) {
            e.toString()
    ...
    fun getYr(timeStamp: Long): String? {
        return try {
            val sdf = SimpleDateFormat("yyyy", Locale.getDefault())//Locale.US for usa
            val netDate = Date(timeStamp*1000)
            sdf.format(netDate)
        } catch (e: Exception) {
            e.toString()
    ...
    fun getTime(timeStamp: Long): String? {
        return try {
            val sdf = SimpleDateFormat("HH:mm a", Locale.getDefault())//Locale.US for usa
            val netDate = Date(timeStamp*1000)
            sdf.format(netDate)
        } catch (e: Exception) {
            e.toString()
